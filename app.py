...
<style>
    ...
    /* =========================================================
       METRIC CARDS
       Keep white background + black/dark text in BOTH themes
       ========================================================= */
    ...
    .drishti-card [data-testid="stMarkdownContainer"],
    .drishti-card [data-testid="stMarkdownContainer"] p,
    .drishti-card [data-testid="stMarkdownContainer"] span,
    .drishti-card [data-testid="stMarkdownContainer"] strong {
        color: #000000 !important;
    }


    /* =========================================================
       INPUTS INSIDE WHITE CARDS
       ========================================================= */
    .drishti-card input,
    .drishti-card textarea {
        color: #000000 !important;
        background-color: #FFFFFF !important;
    }
    ...
    /* =========================================================
       EXPANDER TEXT
       ========================================================= */
    .drishti-card [data-testid="stExpander"] summary,
    .drishti-card [data-testid="stExpander"] summary span {
        color: #000000 !important;
    }


    /* =========================================================
       SELECTBOX / FORM LABELS
       ========================================================= */
    .drishti-card [data-testid="stWidgetLabel"],
    .drishti-card [data-testid="stWidgetLabel"] *,
    .drishti-card [data-testid="stTextInput"] label,
    .drishti-card [data-testid="stNumberInput"] label,
    .drishti-card [data-testid="stSelectbox"] label {
        color: #000000 !important;
    }
...
def login_page():
    ...
    st.markdown(
        f"<div class='drishti-banner'>⚠️ "
        f"{t('prototype_banner', lang)}</div>",
        unsafe_allow_html=True
    )
    ...
    with mid:
        ...
        st.caption(
            "Demo accounts — Health Worker: `healthworker1` / `worker123`  ·  "
...
def sidebar_nav():
    ...
    with st.sidebar:
        ...
        st.markdown(
            f"**{t('welcome', lang)}, {user['display_name']}**"
        )

        st.caption(
            f"{t('role', lang)}: {user['role']}"
        )
        ...
        if IS_PROTOTYPE:
            if dr_model.mode == "mock":
                st.caption(
                    "🧪 " +
                    t("demo_mode_notice", st.session_state.lang)
                )
            else:
                st.caption("✅ Real model loaded.")

        if st.button(
            t("logout", st.session_state.lang),
            use_container_width=True
        ):
...
def page_screening():
    ...
    st.markdown(
        ...
        f"**Age:** {patient['age']}  |  "
        f"**Diabetes duration:** "
        f"{patient['diabetes_duration']}y  |  "
        f"**Previous screening:** "
        f"{patient['previous_screening']}"
    )
    ...
    with col_up:
        ...
        if uploaded_file is not None:
            ...
            st.session_state.wf_image_bgr = pil_to_bgr(
                pil_img
            )
    ...
    with col_sample:
        ...
        for severity in DR_CLASSES:
            fname = (
                f"sample_"
                f"{severity.replace(' ', '_').lower()}.png"
            )
    ...
    if st.session_state.wf_image_bgr is None:
        st.info(
            "Upload a fundus image or pick a demo sample to begin."
        )
        return

    st.image(
        bgr_to_rgb(st.session_state.wf_image_bgr),
        caption=t("image_preview", lang),
        width=320
    )
    ...
    if st.button(
        ...
        st.session_state.wf_quality = check_image_quality(
    ...
    if st.button(
        ...
        dr_class, confidence, probs = dr_model.predict(
            st.session_state.wf_image_bgr
        )
        ...
        st.session_state.wf_heatmap = generate_explanation(
            dr_model,
            st.session_state.wf_image_bgr
        )
...
def page_dashboard():
    ...
    tab_overview, tab_followup = st.tabs(
        [
    ...
    with tab_followup:
        ...
        st.caption(
            "Update follow-up status per screening "
            "(Doctor role)."
        )
